...
import { getPerceptualHash } from "../../../lib/services/peceptual-hash"
...
(async ()  => {
  ...
  const images = await client.$queryRaw`SELECT slug, id, p_hash, p_hash_2 IS NOT NULL as has_phash_2 FROM images`
  for (const image of images.filter(a => a.p_hash && !a.has_phash_2)) {
    ...
    const a = await client.$executeRaw`${Prisma.raw(`
      UPDATE images SET p_hash_2 = CUBE(ARRAY[${code.join(",")}]) WHERE slug = '${image.slug}' RETURNING id
    `)}`
    console.log({ a })
  }
  ...
  for (const image of images.filter(a => !a.p_hash)) {
    ...
    const phash = await getPerceptualHash.applyAsync([Buffer.from(data).toString("base64")]).get()
