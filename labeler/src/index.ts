...
import { colorPalette, phash } from "./ffi.js"
import { backend } from "../../shared/sdk.js"
import fetch from "node-fetch"
import { PERCEPTUAL_HASH_TASK_NAME } from "../../shared/messageQueue.js"
...
const celeryWorker = createWorker(config.get("amqpUrl"), config.get("amqpUrl"))

// TODO: fetch settings? Maybe not all images might be accessible
celeryWorker.register(PERCEPTUAL_HASH_TASK_NAME, async (url: string) => {
  const ab = await fetch(url).then((res) => res.arrayBuffer())
  return phash(Buffer.from(ab))
})
